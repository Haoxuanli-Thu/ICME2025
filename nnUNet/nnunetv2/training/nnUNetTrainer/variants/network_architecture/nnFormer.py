...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None):
        ...
        shifted_x = window_reverse(
            attn_windows, self.window_size, Sp, Hp, Wp
        )  # B H' W' C
        ...
        for i in range(depth - 1):
            self.blocks.append(
                SwinTransformerBlock(
                    ...
                    attn_drop=attn_drop,
                    drop_path=(
                        drop_path[i + 1] if isinstance(drop_path, list) else drop_path
                    ),
                    norm_layer=norm_layer,
                )
            )
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        self.proj1 = project(
            in_chans, embed_dim // 2, stride1, 1, nn.GELU, nn.LayerNorm, False
        )
        self.proj2 = project(
            embed_dim // 2, embed_dim, stride2, 1, nn.GELU, nn.LayerNorm, True
        )
...
class nnFormer(SegmentationNetwork):
    def __init__(
        ...
        if self.do_ds:
            ...
            self.final.append(
                final_patch_expanding(embed_dim, num_classes, patch_size=patch_size)
            )
...
if __name__ == "__main__":
    model = nnFormer(deep_supervision=False)
